...
test_data <- et_data %>% filter(target_audio_type == "novel",
                                ...
                                look_left_img == TRUE | 
                                  look_right_img == TRUE,
                                time_since_trial_start >= start_event_time &  time_since_trial_start <= 26300)
```
...
summarise(m_prop_looks_action = mean(unique(subj_prop_look_action)),
          ...
          CI.Low = ci.low(unique(subj_prop_look_action)),
          CI.High = ci.high(unique(subj_prop_look_action)), n = length(unique(participant_id))) %>%
  mutate(YMin = m_prop_looks_action - CI.Low, 
         YMax = m_prop_looks_action + CI.High)
```
...
```{r}
test_action_sum_trial <- test_data %>% group_by(condition, trial_name) %>% 
summarise(m_prop_looks_action = mean(unique(proportion_look_action)),
          ...
          CI.High = ci.high(unique(proportion_look_action)), n = length(unique(proportion_look_action))) %>%
  mutate(YMin = m_prop_looks_action - CI.Low, 
         YMax = m_prop_looks_action + CI.High)
```
...
```{r}
test_action_sum_item <- test_data %>% group_by(condition, img_pair_name) %>% 
summarise(m_prop_looks_action = mean(unique(proportion_look_action)),
          sd = sd(unique(proportion_look_action)),
          CI.Low = ci.low(unique(proportion_look_action)),
          CI.High = ci.high(unique(proportion_look_action)), n = length(unique(proportion_look_action))) %>%
...
```{r}
test_data <- test_data %>% mutate(timebin = plyr::round_any(time_since_trial_start, 50))
```

Plot the timecourse data by trial number.
```{r}
prop_action_timecourse_sum_trial <- test_data %>% group_by(timebin, condition, trial_name) %>% 
...
ggplot(prop_action_timecourse_sum_trial) +
  ...
  geom_vline(xintercept = 23500)
ggsave(file="../graphs/overall_timecourse_trial.pdf",width=8,height=5)
```

Plot the timecourse collapsing by trial number.
```{r}
prop_action_timecourse_sum <- test_data %>% group_by(timebin, condition) %>% 
  summarise(timebin_prop_action_look = mean(look_action_img))
...
ggplot(prop_action_timecourse_sum) +
  ...
  geom_vline(xintercept = 21000) +
  geom_vline(xintercept = 23500)
ggsave(file="../graphs/overall_timecourse.pdf",width=8,height=5)
```
...
clicked_img_sum <- clicked_img_sum %>% mutate(proportion = n/total)
```
